...
public:
  ...
  TransformFusion() {
    ...
    subLaserOdometry = nh.subscribe<nav_msgs::Odometry>(
        "lio_sam/mapping/odometry", 5, &TransformFusion::lidarOdometryHandler, this,
        ros::TransportHints().tcpNoDelay());
    subImuOdometry = nh.subscribe<nav_msgs::Odometry>(
        odomTopic + "_incremental", 2000, &TransformFusion::imuOdometryHandler, this,
        ros::TransportHints().tcpNoDelay());
  ...
  void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg) {
    ...
    tfMap2Odom.sendTransform(tf::StampedTransform(map_to_odom, odomMsg->header.stamp,
                                                  mapFrame, odometryFrame));
...
public:
  ...
  IMUPreintegration() {
    subImu =
        nh.subscribe<sensor_msgs::Imu>(imuTopic, 2000, &IMUPreintegration::imuHandler,
    ...
    priorBiasNoise =
        gtsam::noiseModel::Isotropic::Sigma(6, 1e-3); // 1e-2 ~ 1e-3 seems to be good
    ...
    noiseModelBetweenBias = (gtsam::Vector(6) << imuAccBiasN, imuAccBiasN, imuAccBiasN,
